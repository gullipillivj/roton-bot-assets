async function simulateCycle(cycleNum) {
    try {
        ...
        // ✅ Allow up to 3 hops per cycle with 0.45% fee
        const feeRate = 0.0045; // 0.45%
        const totalFeeFactor = 1 - feeRate;

        for (let hop = 1; hop <= 3; hop++) {
            // Apply fee internally
            ...
            logWithTime(`[Latest] Hop ${hop}: into ${coin}, Units=${coinUnits.toFixed(4)}, Entry=${entryPrice.toFixed(4)} USDT after 0.45% fee`);
            ...
            // ✅ Only update textboxes after profit/loss
